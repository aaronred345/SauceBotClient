...
public enum IrcCode {
    ...
    IrcCode(final int code) {
        this(code, false);
    }

    IrcCode(final int code, final boolean isError) {
        this.code = code;
    ...
    private static Map<Integer, IrcCode> codeMap;

    public static IrcCode getForCode(final int code) {
        buildMapIfNeeded();
        IrcCode ircCode = codeMap.get(code);
        return ircCode != null ? ircCode : Unknown;
    }

    private static void buildMapIfNeeded() {
        if (codeMap != null) {
            return;
        }
        codeMap = new HashMap<Integer, IrcCode>();
